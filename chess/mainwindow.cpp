...
void MainWindow::blackTurn()
{
    ...
    Move m = situation->solveRec(0, 1, level).first;
    moves.push_back(m);
    writeMoves();
    ...
    //figures[m.x1][m.y1]->showMove(m.x2, m.y2);
    //ui->label->setText(QString::number(m.x1) + QString::number(m.y1) + QString::number(m.x2) + QString::number(m.y2));
    showBoard(1, m);
